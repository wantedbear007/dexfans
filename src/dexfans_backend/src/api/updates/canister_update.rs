...
#[ic_cdk::update(guard = guard_post_canister_exclusive)]
pub fn ic_unsubscribe_account(args: dexfans_types::types::UnsubscribeAccountIC) -> Result<(), String> {
    crate::with_write_state(|state| {
        ...
        unsubscribed_by.subscribing.retain(|&subscribed_user_id| subscribed_user_id != unsubscribed_to.user_id);
        
        // Remove the unsubscribed_by from the unsubscribed_to's subscribers
        unsubscribed_to.subscribers.retain(|&subscriber_id| subscriber_id != unsubscribed_by.user_id);

        // Update the state with the modified profiles
        state.account.insert(unsubscribed_by.user_id, unsubscribed_by);
        state.account.insert(unsubscribed_to.user_id, unsubscribed_to);
    });
